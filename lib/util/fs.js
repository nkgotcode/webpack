...
/**
 * Converts a `file:` URL (a `URL` instance or a `"file:"` string) to an
 * absolute path. Any other value (plain path, `false`, `RegExp`, ...) is
 * returned unchanged, so it is safe to apply to mixed path options.
 * @template T
 * @param {T} value a path, a `"file:"` URL string, or a file `URL` instance
 * @returns {string | T} an absolute path for file URLs, otherwise `value`
 */
const maybeFileURLToPath = (value) => {
...
module.exports.maybeFileURLToPath = maybeFileURLToPath;
