...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const resolveResourceHints = ResourceHintPlugin.getCompilationResolver(
			state.compilation
		);
		/**
		 * Sets `output.resourceHints` defaults and `webpackPrefetch` /
		 * `webpackPreload` / `webpackFetchPriority` magic-comment overrides
		 * on a fresh `CssUrlDependency`. The magic comment wins over the
		 * project-wide default when both are set.
		 * @param {CssUrlDependency} dep dep
		 * @param {string} request asset request
		 * @param {(request: string) => { prefetch?: boolean, preload?: boolean, fetchPriority?: "low" | "high" | "auto" | false }} resolve resolver
		 ...
		 * @returns {void}
		 */
		const applyResourceHintDefaults = (dep, request, resolve, options, loc) => {
			const defaults = resolve(request);
			if (defaults.prefetch) dep.prefetch = true;
			if (defaults.preload) dep.preload = true;
			if (defaults.fetchPriority) {
				dep.fetchPriority = defaults.fetchPriority;
			}
			if (options) {
				const hints = parseResourceHintOptions(options, state.module, loc);
		...
		const processImportOrExport = (type, second, rule) => {
			...
			return skipWhiteLine(source, rule.blockRange[1]);
		};
		...
		const visitors = {
			...
			Url: (node) => {
				...
				applyResourceHintDefaults(dep, value, resolveResourceHints, options, {
					start: { line: sl, column: sc },
					end: { line: el, column: ec }
				});
				dep.setLoc(sl, sc, el, ec);
