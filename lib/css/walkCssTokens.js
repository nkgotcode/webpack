...
function consumeAStringToken(input, pos) {
	...
	for (;;) {
		...
		if (cc === CC_REVERSE_SOLIDUS) {
			...
			if (pos === input.length) return { type: TT_STRING, start, end: pos };
 ...
 * `undefined` only at end-of-input (`pos >= length`), which the caller turns
 * into `<eof-token>`; unterminated comments/strings still emit a token to EOF
 * so token ranges cover all input. This is the non-generator core the
 * `tokenize` generator and `next` share, so the hot path allocates no
 * per-token iterators.
 * @param {string} input input
...
function readToken(input, pos) {
	...
	if (
		...
		for (;;) {
			...
			if (pos === input.length) return { type: TT_COMMENT, start, end: pos };
			if (
