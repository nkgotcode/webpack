...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		compiler.outputPath = maybeFileURLToPath(options.output.path);
		compiler.recordsInputPath = options.recordsInputPath
			? maybeFileURLToPath(options.recordsInputPath)
			: null;
		compiler.recordsOutputPath = options.recordsOutputPath
			? maybeFileURLToPath(options.recordsOutputPath)
			: null;
		...
		new AddManagedPathsPlugin(
			...
			(options.snapshot.managedPaths).map(maybeFileURLToPath),
			/** @type {NonNullable<WebpackOptions["snapshot"]["managedPaths"]>} */
			(options.snapshot.immutablePaths).map(maybeFileURLToPath),
			/** @type {NonNullable<WebpackOptions["snapshot"]["managedPaths"]>} */
			(options.snapshot.unmanagedPaths).map(maybeFileURLToPath)
		).apply(compiler);
						...
						case "pack": {
							...
							new IdleFileCachePlugin(
								new PackFileCacheStrategy({
									...
									cacheLocation: maybeFileURLToPath(
										/** @type {string} */
